...
protected function convertIncomingQBtoQuery(Builder $query, stdClass $rule, $value, $queryCondition = 'AND')
{
    ...
    */
    ...
    /*$newQuery = Customers::select('*', 'companies.company_name', 'users.name')    
    //->with('getcompanies') //getcompanies relation ship in customer model
    //->with('getcreated')  //getcreated relation ship in customer model
    ->where($rule->field, $sqlOperator['operator'], $value, $condition);    
    return $newQuery; */
    
    //$query->with('getcompanies');
